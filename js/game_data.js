...
const GameConfig = {
    tempo: {
        ...
        // 画面遷移時の安全マージン（標準: 200 -> 高速化: 100）
        transitionBuffer: 100,
        
        // スチル（一枚絵）のパン・ズーム演出時間（標準: 1500 -> 高速化: 800）
        stillAnimDuration: 800,

        // --- エンディング・BGM関連 (ここは演出重視で長めに設定) ---
        
        // BGMのフェードアウト時間（プツンと切れないよう、画面フェード(300)より長くする）
        bgmFadeLong: 1000,

        // エンディング遷移時の暗転待機時間
        // ※showEndingの描画負荷で暗転がスキップされるのを防ぐため、長めの時間を確保する
        // (1000msだと環境によって一瞬描画漏れするため、安全策で2000ms)
        endingFadeWait: 2000,

        // デバッグ機能での強制エンディング待機時間
        // (演出スキップ前提だが、最低限の暗転は確保する)
        debugEndingWait: 1500,

        // 進行不能防止のフェイルセーフ時間
        // (モノローグ演出などが万が一固まっても、この時間が経てば強制遷移する)
        // ※目安: stillAnimDuration(800) + 文字送り時間よりも十分に長く設定すること
        endingFailSafe: 6000
    }
};